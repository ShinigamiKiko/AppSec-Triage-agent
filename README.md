...
Чтобы пройти цепочку, нужны исходники самих пакетов. Сначала читается то, что
лежит в проекте: `vendor/`, `node_modules/`, `site-packages`, кеш модулей Go.
Если нужного пакета там нет, точная версия из лок-файла скачивается и читается
в памяти — npm с `registry.npmjs.org`, Go с `proxy.golang.org`. Загруженное
сверяется с контрольной суммой (у Go — с `h1:` из `go.sum` проекта, иначе из
базы контрольных сумм), на диск проекта ничего не пишется и не выполняется.
Для Composer загрузки нет и она не нужна: джоба сама делает `composer install`,
так что `vendor/` к моменту запуска полный.
