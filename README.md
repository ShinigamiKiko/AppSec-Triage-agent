...
диска: `vendor/`, `node_modules/`, `site-packages`, кеш модулей Go. `run` по
умолчанию ставит npm- и composer-зависимости сам — в копию проекта, по версиям
из lock-файла, из публичных источников, даже если lock-файл указывает на
внутренний прокси (satis, Nexus); `--no-install-deps` сканирует дерево как есть.
Если пакета на диске нет (исходники только на приватном хосте), цепочка через
него не проходит и так и пишет об этом; тот же каталог нужен языковому серверу,
чтобы разрешать типы внутри зависимости.
