...
Чтобы пройти цепочку, нужны исходники самих пакетов, и агент читает их с
диска: `vendor/`, `node_modules/`, `site-packages`, кеш модулей Go. Ставит их
джоба — своими реестрами, к которым у неё есть доступ (satis, Nexus):

```bash
composer install --no-dev --no-scripts --no-plugins --ignore-platform-reqs --no-interaction
```

`--no-dev` обязателен: dev-инструменты проекта (свой Psalm, phpstan) в `vendor/`
ломают Psalm агента, а в рабочий образ не попадают. Для npm — `npm ci` или
`yarn install --frozen-lockfile` (реестр — из `.npmrc`/`.yarnrc` проекта), для
Go — `go mod download` с `GOPROXY`/`GOPRIVATE` джобы: govulncheck, CodeQL и gopls
агента берут их из окружения. Если пакета на диске нет, цепочка через него не
проходит и так и пишет об этом; тот же каталог нужен языковому серверу, чтобы
разрешать типы внутри зависимости.

Вне сети CI (локальные тесты) `--install-deps` ставит npm- и composer-зависимости
в копию проекта из публичных источников: адреса приватного прокси в lock-файле
переписываются на публичный реестр и GitHub, версии остаются прежними.

Разбор advisory (уязвимая функция, условие эксплуатации) зависит только от
advisory, версии пакета, модели и промтов, поэтому хранится между прогонами —
по умолчанию в `<out>/.appsec-cache`, или там, куда указывает `APPSEC_CACHE_DIR`
(пустое значение выключает). В CI этот каталог стоит отдать в `cache:`. Внутри
прогона Psalm кеширует ответы по каждому методу всегда, флагов для этого нет.
